...
- [ ] تفعيل بوابة الدفع الإلكتروني فعلياً (يتطلب حساب Moyasar تجاري + واجهة تحصيل بطاقة Moyasar.js)
...
| بوابة الدفع Moyasar | البنية الخلفية جاهزة (`server.ts`) | إنشاء حساب Moyasar تجاري، ضبط `MOYASAR_SECRET_KEY`/`MOYASAR_WEBHOOK_SECRET`، وبناء واجهة تحصيل البطاقة بـ Moyasar.js |
...
| اختبارات E2E | غير موجودة | لا تزال أعلى فجوة في تغطية الاختبارات (P9-testing.md) — يُنصح بإضافة Playwright لمسارات: تسجيل الدخول، إنشاء عميل، فتح قضية، إصدار فاتورة |
