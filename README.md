...
- [ ] تفعيل بوابة الدفع الإلكتروني (Moyasar)
- [ ] الإشعارات البريدية الفعلية (SendGrid/Resend)
...
- **Facade Pattern:** خدمة AI تخفي التعقيد (API حقيقي أو Mock)
...
1. **server.js** يستخدم Firebase Admin لـ Auth verification ← المشروع انتقل إلى Supabase ← قد يحتاج تحديث
2. **بعض الـ endpoints** في server.js تستخدم `admin.firestore()` ← يجب الانتقال إلى Supabase
3. **الحقول المشفرة** تُخزن كـ Base64 في PostgreSQL ← لا يمكن البحث فيها مباشرة
4. **MOYASAR_PAYMENTS** معطّل ← بوابة الدفع mock فقط حالياً
5. **بعض fetch functions** (fetchTrustAccounts, fetchEnforcement, fetchTasks, fetchTeam) تعيد مصفوفة فارغة ← تحتاج تنفيذ فعلي
6. **الـ CRM view** موجود كملف لكن غير مسجل في App.tsx routes
